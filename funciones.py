...
def preprocesamiento_datos(df: pd.DataFrame) -> pd.DataFrame:
    """
    Realiza la limpieza, el feature engineering (inasistencia, edad, espera, turno),
    el filtrado IQR, identifica los NAs y selecciona las columnas finales para el modelo.
    """
    df = df.copy()
    
   # 1. INICIALIZACIÓN DE VARIABLES PARA ROBUSTEZ
    df['inasistencia'] = np.nan
    df['grupo_etario'] = 'missing'
    df['dias_espera'] = -1
    df['turno_cita'] = 'missing'
    
    # 2. CREACIÓN DE INASISTENCIA (Depende de 'Estado Cita')
    ...
    # 5. CREACIÓN DE TURNO DE CITA (Depende de 'Hora Inicio Cita')
    ...
    # 7. FILTRADO DE OUTLIERS POR RANGO INTERCUARTÍLICO (IQR)
    ...
    # 8. PREPARACIÓN E IDENTIFICACIÓN DE NULOS
...
def aplicar_transformaciones_finales(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.Series, random_state=42):
    ...
    # 3. RECONSTRUCCIÓN Y CORRECCIÓN DE NOMBRES (SOLUCIÓN ROBUSTA LightGBM)
    ...
    for name in feature_names:
        ...
        # 3b. SOLUCIÓN ROBUSTA: Reemplazar cualquier caracter que NO sea alfanumérico o guion bajo (\w) por un guion bajo
        # Esto elimina de forma agresiva TODOS los caracteres problemáticos (espacios, corchetes, :, <, etc.)
        cleaned_name = re.sub(r'[^\w]+', '_', cleaned_name)
        
        # 3c. Eliminar guiones bajos sobrantes al inicio o final de la cadena
        cleaned_name = cleaned_name.strip('_')
        
        # 3d. Forzar minúsculas (buena práctica)
        cleaned_name = cleaned_name.lower()

        # 3e. Caso de contingencia: Si el nombre empieza con un dígito (raro en este caso, pero seguro), añadir prefijo
...
# Colocar esta función en tu archivo funciones.py, reemplazando la anterior
...
def evaluar_segmento(modelo, X_test_ohe, y_test, X_test_original, nombre_columna, umbral=0.36):
    ...
    # Nota: Asumimos que X_test_ohe y y_test ya tienen el mismo índice que X_test_original 
    # (por la corrección que hicimos en la celda del ColumnTransformer).
...
## 7. Visualización del Rendimiento Segmentado
...
def crear_grafico_segmento(df_segmentos, segmento_col, umbral_valor, metrica='Recall (R)', color='Reds_d'):
    ...
    # 2. Ajustar el tamaño de la figura dinámicamente si hay muchas categorías
    fig_height = max(5, len(df_plot) * 0.4) 
    plt.figure(figsize=(10, fig_height))
    
    # 3. Generar el gráfico de barras comparativo (CORRECCIÓN WARNING)
    ax = sns.barplot(
        x=metrica, 
        y='Valor', 
        data=df_plot, 
    ...
    # 5. Título CORREGIDO para usar el valor de 'umbral_valor'
...
def optimizar_modelo_con_grid_search(modelo_base, X_train, y_train, param_grid, cv=5, scoring='roc_auc'):
    ...
    grid_search = GridSearchCV(
        ...
        scoring=scoring,  # Usar AUC-ROC como métrica principal
        cv=cv,            # Validación cruzada (5 folds)
        verbose=1,
...
def graficar_curvas_umbral(modelo, X_test_ohe, y_test, titulo=""):
    ...
    # Calcular F1-score para cada umbral (excepto el último, que no tiene umbral)
...
def generar_alertas_clinicas(modelo, X_test, y_test, X_base_df, umbral_operacional: float):
    ...
    # Unir con las variables originales (sin transformar ni normalizar) para la acción clínica
    # Nota: X_base_df es el df original antes de OHE/Normalización.
...
def analisis_importancia_shap(modelo, X_test):
    ...
    try:
        ...
        shap_values_clase1 = None # No se pudo calcular la tabla SHAP
